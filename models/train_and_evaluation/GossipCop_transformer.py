...
# neighbor_dict = neighbor_loader('fnn_gossipcop_50/n_neighbors.txt')

neighbor_dict = neighbor_loader('data/processed_data/FakeNewsNet/GossipCop/gossipcop_n_neighbors.txt')

def data_loader(pathway='data/processed_data/FakeNewsNet/GossipCop/batch/normalized_post_nodes/', node_type="post"):
    if node_type == "news":      
        news_node = []
        news_id = []
        news_label = []
...
class HetTransformer(nn.Module):
    ...
    def __init__(self, input_dim, ini_hidden_dim, hidden_dim,
                 ...
                 enc_layers=1, npu=30, self_attn_heads=2):
...
#                                                  bidirectional=True, dropout=0.5)
    ...
    def transformer_agg(self, het_node, neighbor_order_n_p_u, neighbor_order_n, npu=30, n=5):  # heterogeneous neighbor aggregation
        # attention module
...
num_folds = 1
batch_size = 4
PATH = 'models/pre-trained/GossipCop/'
...
for fold in range(num_folds):
    ...
    net = HetTransformer(input_dim=[768, 512, 3, 29], ini_hidden_dim=[200, 200, 200, 200], hidden_dim=100,
                  ...
                  use_gcn=True, enc_layers=1, feed_forward=512, npu=30)
    net.init_weights()
    print(net)
...
for count in range(num_folds):
    cur_PATH = PATH + 'best_model' + '_' + str(count) + '_99.tar'
    net, optimizer, epoch, best_val = load_checkpoint(net, optimizer, cur_PATH)
    print(best_val)
    net.eval()
