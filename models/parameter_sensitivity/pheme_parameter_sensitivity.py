...
# Load the node neighbors, save them into dictionary
...
post_nodes = data_loader(pathway='F:/pheme_input_200/pheme_input/transformer_200/normalized_post_nodes/', node_type="post")
news_nodes = data_loader(pathway='F:/pheme_input_200/pheme_input/transformer_200/normalized_news_nodes/', node_type="news")
user_nodes = data_loader(pathway='F:/pheme_input_200/pheme_input/transformer_200/normalized_user_nodes/', node_type="user")
news_emb_dict = {}
post_emb_dict = {}
user_emb_dict = {}
...
class Het_GNN(nn.Module):
    ...
    def __init__(self, input_dim, ini_hidden_dim, hidden_dim,
                 ...
                 p_rnn_type='LSTM',
                 rnn_type='LSTM', embed_d=200, symmetry=False, GCN_out2=300, GCN_out1=600, GCN_in=200, 
                 use_bias=True, attn_heads = 2, d_model = 512, self_attn_heads = 1, enc_layers=3, dec_layers=3, npu = 15):
        ...
        self.transformer = nn.Transformer(d_model=d_model, nhead=attn_heads, num_encoder_layers=dec_layers, num_decoder_layers=enc_layers, dim_feedforward=256, 
    ...
    def GCN_net(self, u_aft_rnn_dict, p_aft_rnn_dict, n_aft_rnn_dict):
        ...
        gcn2_user, gcn2_post, gcn2_news = self.GCN_layer(user_A, user_D, gcn1_user, post_A, post_D, gcn1_post,
                                                           news_A,
                                                           news_D, gcn1_news, 2)
    ...
    def transformer_agg(self, het_node, neighbor_order_n_p_u, neighbor_order_n, npu=3, n=5):  # heterogeneous neighbor aggregation
        # attention module
    ...
    def node_het_agg(self, het_node):  # heterogeneous neighbor aggregation
        ...
        u_agg_batch_2 = torch.mean(u_agg_batch_2, 0)
        u_agg_batch_2 = u_agg_batch_2.view(1, u_agg_batch_2.shape[0])
        p_agg_batch_2 = torch.cat((gcn2_post, p_agg_batch_2), 0)
        p_agg_batch_2 = torch.mean(p_agg_batch_2, 0)
        p_agg_batch_2 = p_agg_batch_2.view(1, p_agg_batch_2.shape[0])
        n_agg_batch_2 = torch.cat((gcn2_news, n_agg_batch_2), 0)
        n_agg_batch_2 = torch.mean(n_agg_batch_2, 0)
        n_agg_batch_2 = n_agg_batch_2.view(1, n_agg_batch_2.shape[0])
...
for npu in npus:
    for fold in range(num_folds):
        ...
        test_loss = 0.0
        best_epoch = 0
        net = Het_GNN(input_dim=[768, 22], ini_hidden_dim=[200, 200], hidden_dim=100,
                          n_hidden_dim=hidden_dims[0], n_ini_hidden_dim=128, n_output_dim=150,
                          u_hidden_dim=hidden_dims[0], u_ini_hidden_dim=128, u_output_dim=150,
                          p_hidden_dim=hidden_dims[0], p_ini_hidden_dim=128, p_output_dim=150,
                          out_embed_d=outemb_dims[0], outemb_d=1, attn_heads=8, d_model=outemb_dims[0], 
                      self_attn_heads=8, enc_layers=1, dec_layers=1, npu=npu)
        net.init_weights()
        print(net)
...
net = Het_GNN(input_dim=[768, 22], ini_hidden_dim=[200, 200], hidden_dim=100,
                          ...
                          p_hidden_dim=hidden_dims[0], p_ini_hidden_dim=128, p_output_dim=150,
                          out_embed_d=outemb_dims[0], outemb_d=1, attn_heads=8, d_model=outemb_dims[0], 
                      self_attn_heads=8, enc_layers=1, dec_layers=1, npu=15)
net.init_weights()
# Set up optimizer and scheduler
optimizer = optim.SGD(net.parameters(), lr=lr)
for npu in npus:
